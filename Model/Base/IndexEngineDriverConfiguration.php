...
abstract class IndexEngineDriverConfiguration implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    ...
    /**
     * The value for the serialized_configuration field.
     * @var        string
     */
    protected $serialized_configuration;

    /**
     * @var        ObjectCollection|ChildIndexEngineIndex[] Collection to store aggregation of ChildIndexEngineIndex objects.
     */
    protected $collIndexEngineIndices;
    ...
    /**
     * Compares this with another <code>IndexEngineDriverConfiguration</code> instance.  If
     * <code>obj</code> is an instance of <code>IndexEngineDriverConfiguration</code>, delegates to
     * <code>equals(IndexEngineDriverConfiguration)</code>.  Otherwise, returns <code>false</code>.
     *
    ...
    public function setSerializedConfiguration($v)
    {
        ...
        if ($this->serialized_configuration !== $v) {
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildIndexEngineDriverConfigurationQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @see IndexEngineDriverConfiguration::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        try {
            $deleteQuery = ChildIndexEngineDriverConfigurationQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                IndexEngineDriverConfigurationTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->indexEngineIndicesScheduledForDeletion !== null) {
                if (!$this->indexEngineIndicesScheduledForDeletion->isEmpty()) {
                    \IndexEngine\Model\IndexEngineIndexQuery::create()
                        ->filterByPrimaryKeys($this->indexEngineIndicesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->indexEngineIndicesScheduledForDeletion = null;
                }
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $sql = sprintf(
            'INSERT INTO index_engine_driver_configuration (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'SERIALIZED_CONFIGURATION':
                        $stmt->bindValue($identifier, $this->serialized_configuration, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    {
        $pos = IndexEngineDriverConfigurationTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['IndexEngineDriverConfiguration'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['IndexEngineDriverConfiguration'][$this->getPrimaryKey()] = true;
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getIndexEngineIndices() as $relObj) {
    ...
    /**
     * Clears out the collIndexEngineIndices collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return void
     * @see        addIndexEngineIndices()
     */
    public function clearIndexEngineIndices()
    {
        $this->collIndexEngineIndices = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collIndexEngineIndices collection loaded partially.
     */
    public function resetPartialIndexEngineIndices($v = true)
    {
        $this->collIndexEngineIndicesPartial = $v;
    }

    /**
     * Initializes the collIndexEngineIndices collection.
     *
     * By default this just sets the collIndexEngineIndices collection to an empty array (like clearcollIndexEngineIndices());
     ...
     * @return void
     */
    public function initIndexEngineIndices($overrideExisting = true)
    {
        if (null !== $this->collIndexEngineIndices && !$overrideExisting) {
            return;
        }
        $this->collIndexEngineIndices = new ObjectCollection();
        $this->collIndexEngineIndices->setModel('\IndexEngine\Model\IndexEngineIndex');
    }

    /**
     * Gets an array of ChildIndexEngineIndex objects which contain a foreign key that references this object.
     *
     ...
     * @throws PropelException
     */
    public function getIndexEngineIndices($criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collIndexEngineIndicesPartial && !$this->isNew();
        if (null === $this->collIndexEngineIndices || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collIndexEngineIndices) {
                // return empty collection
                $this->initIndexEngineIndices();
            } else {
                $collIndexEngineIndices = ChildIndexEngineIndexQuery::create(null, $criteria)
                    ->filterByIndexEngineDriverConfiguration($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collIndexEngineIndicesPartial && count($collIndexEngineIndices)) {
                        $this->initIndexEngineIndices(false);

                        foreach ($collIndexEngineIndices as $obj) {
                            if (false == $this->collIndexEngineIndices->contains($obj)) {
                                $this->collIndexEngineIndices->append($obj);
                            }
                        }

                        $this->collIndexEngineIndicesPartial = true;
                    }

                    reset($collIndexEngineIndices);

                    return $collIndexEngineIndices;
                }

                if ($partial && $this->collIndexEngineIndices) {
                    foreach ($this->collIndexEngineIndices as $obj) {
                        if ($obj->isNew()) {
                            $collIndexEngineIndices[] = $obj;
                        }
                    }
                }

                $this->collIndexEngineIndices = $collIndexEngineIndices;
                $this->collIndexEngineIndicesPartial = false;
            }
        }

        return $this->collIndexEngineIndices;
    }

    /**
     * Sets a collection of IndexEngineIndex objects related by a one-to-many relationship
    ...
    public function setIndexEngineIndices(Collection $indexEngineIndices, ConnectionInterface $con = null)
    {
        ...
        foreach ($indexEngineIndicesToDelete as $indexEngineIndexRemoved) {
            $indexEngineIndexRemoved->setIndexEngineDriverConfiguration(null);
        }

        $this->collIndexEngineIndices = null;
        foreach ($indexEngineIndices as $indexEngineIndex) {
            $this->addIndexEngineIndex($indexEngineIndex);
        }

        $this->collIndexEngineIndices = $indexEngineIndices;
        $this->collIndexEngineIndicesPartial = false;
    ...
    public function countIndexEngineIndices(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collIndexEngineIndices || null !== $criteria || $partial) {
            ...
            if ($partial && !$criteria) {
                return count($this->getIndexEngineIndices());
            }

            $query = ChildIndexEngineIndexQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collIndexEngineIndices);
    }
    ...
    public function addIndexEngineIndex(ChildIndexEngineIndex $l)
    {
        ...
        if (!in_array($l, $this->collIndexEngineIndices->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddIndexEngineIndex($l);
        }
    ...
    /**
     * @param IndexEngineIndex $indexEngineIndex The indexEngineIndex object to add.
     */
    ...
    {
        if ($this->getIndexEngineIndices()->contains($indexEngineIndex)) {
            $this->collIndexEngineIndices->remove($this->collIndexEngineIndices->search($indexEngineIndex));
            if (null === $this->indexEngineIndicesScheduledForDeletion) {
    ...
    {
        if ($deep) {
            if ($this->collIndexEngineIndices) {
                foreach ($this->collIndexEngineIndices as $o) {
    ...
    {
        return (string) $this->exportTo(IndexEngineDriverConfigurationTableMap::DEFAULT_STRING_FORMAT);
    }
